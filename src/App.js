  ...
  const generate = async () => {
    ...
    const res = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: 'user',
          content: `You are Dan Quirk, replying professionally to a ${type}.
...
Write a professional response.`
  ...
  return (
    <div style={{ fontFamily: 'Arial', padding: '2rem', maxWidth: '800px', margin: 'auto' }}>
      ...
      <textarea rows={10} placeholder='Paste email here...' value={input} onChange={(e) => setInput(e.target.value)} style={{ width: '100%', marginTop: '1rem', padding: '1rem', fontSize: '1.1rem' }}/>
      <button onClick={generate} disabled={loading} style={{ marginTop: '1rem', padding: '1rem 2rem', backgroundColor: '#25644A', color: '#fff', fontSize: '1.1rem', border: 'none' }}>
